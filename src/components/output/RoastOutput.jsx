...
const RoastOutput = ({ result, isLoading, mode = 'savage' }) => {
    ...
    return (
      <div className="flex flex-col items-center justify-center p-10 glass-panel bg-slate-900/40 border-slate-800/60 rounded-[2rem] animate-in fade-in zoom-in duration-1000 shadow-2xl group hover:border-slate-700/50 transition-all">
        <div className="relative mb-8 group-hover:scale-110 transition-transform duration-500">
          <div className="absolute inset-0 bg-gradient-to-br from-violet-500/30 to-rose-600/30 blur-3xl rounded-full scale-150 animate-pulse" />
          <div className="relative w-20 h-20 rounded-3xl bg-slate-800/80 flex items-center justify-center border border-slate-700/50 shadow-2xl rotate-3">
             <Flame size={40} className="text-violet-400 animate-bounce" />
          </div>
        </div>
  ...
  return (
    <div className={`flex flex-col rounded-3xl border border-slate-700/60 bg-slate-900/40 backdrop-blur-xl shadow-[0_8px_30px_rgba(0,0,0,0.35)] overflow-hidden relative group ${
      mode === 'savage' ? 'hover:border-rose-500/20' : 'hover:border-cyan-500/20'
      ...
      <div className={`absolute top-0 right-0 w-64 h-64 blur-3xl opacity-10 pointer-events-none transition-colors duration-1000 ${
        mode === 'savage' ? 'bg-rose-500' : 'bg-cyan-500'
      }`} />
      ...
      <div className="flex items-center justify-between px-6 py-5 border-b border-slate-800/80 bg-slate-900/40 backdrop-blur-md">
        <div className="flex items-center gap-4">
          <div className={`p-2.5 rounded-2xl border ${
            mode === 'savage' ? 'bg-rose-500/10 border-rose-500/20 text-rose-400' : 'bg-cyan-500/10 border-cyan-500/20 text-cyan-400'
          } shadow-lg`}>
          ...
          <div>
            ...
            <div className="flex items-center gap-2 mt-1">
              <span className={`w-2 h-2 rounded-full animate-pulse shadow-[0_0_8px_rgba(244,63,94,0.5)] ${
                isLoading ? 'bg-rose-500' : 'bg-cyan-500'
              }`} />
        ...
        {isLoading ? (
          ...
          <div className="animate-in fade-in slide-in-from-bottom-2 duration-700">
            <article className={`prose prose-invert max-w-none 
              ...
              ${mode === 'savage' ? 'prose-headings:text-rose-400' : 'prose-headings:text-cyan-400'}
